...
const Login = () => {
  ...
  const handleLogin = async() => {
    ...
    try{
      ...
      const response = await axios.post('http://localhost:5000/login',{email: email, password: password}); //awati: wait till server response
        ...
        <div className='inputs'>
          <div className='input'>
            ...
            <input type="email" 
              ...
              value={email} 
              onChange={(e) => setEmail(e.target.value)}/> {/*On change works every time an new character is entered or altered, e means event*/}
          </div>
